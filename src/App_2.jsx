...
export default function App(){
  ...
  const push=useCallback(n=>{
    ...
    setHf(null);setHd(null);setHa(null);
  ...
  const goHome=useCallback(()=>{
    ...
    setHf(null);setHd(null);setHa(null);setStk([]);setSi(0);
    setLogoS(false);
  ...
  const openFilmInMain=useCallback(f=>{
    ...
    setHf(null);setHd(null);setHa(null);setSi(0);
  ...
  const crossHover = (mode==="director"&&(ha||(hd&&hd!==ad))) || (mode==="actor"&&(hd||(ha&&ha!==aa))) || (isSnap&&(hd||ha));

  const hl=useMemo(()=>{
    if(mode==="main"){
      // Director/actor hover takes priority over activeFilm
  ...
  },[mode,activeFilm,hf,hd,ha,cur,ad,aa]);
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#222",color:"#ccc",fontFamily:"'JetBrains Mono',monospace",fontSize:FS,display:"grid",gridTemplateColumns:"200px 1fr 50px 250px",gridTemplateRows:"1fr",overflow:"hidden"}}>
      ...
      <div onMouseEnter={()=>setHz("dirs")} onMouseLeave={()=>setHz(null)}
        ...
        {sD.map(d=>{
          ...
          return(<div key={d} style={{transition:"opacity .4s",opacity:o}}>
            ...
            {isOwner&&!crossHover&&<div style={{fontSize:FS,opacity:.35,lineHeight:"12px",fontWeight:300}}>{DIR_INFO[d]||""}</div>}
          </div>);
        })}
      </div>
      ...
      <div style={{gridColumn:2,overflow:"hidden",display:"flex",flexDirection:"row"}}>
        ...
        <div onMouseEnter={()=>setHz("bars")} onMouseLeave={()=>setHz(null)}
          style={{width:160,flexShrink:0,display:"flex",flexDirection:"column",justifyContent:"center",padding:"8px 0",transition:"background .3s",background:hz==="bars"?"rgba(255,255,255,.02)":"transparent"}}>
          {isSub?(
              ...
              {FILMS.map((f,i)=>{
                ...
                return(
                  <div key={f.id} style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end",flex:1,transition:"opacity .4s",opacity:isH?.9:(has?.04:.12),paddingRight:8,minHeight:0,cursor:"pointer"}}
                    ...
                    onClick={()=>{
                      if(isActive){setActiveFilm(null);}
              ...
              {sub.map(f=>(
                <div key={f.id} style={{height:"100%",minHeight:"100%",scrollSnapAlign:"start",display:"flex",flexDirection:"column",padding:"8px 12px",overflow:"auto"}}>
                  <div style={{width:"100%",aspectRatio:"16/9",background:f.color,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}} onClick={()=>openFilmInMain(f)}>
                    <div style={{fontSize:FS,fontWeight:700,color:"rgba(255,255,255,.18)",textTransform:"uppercase",letterSpacing:1}}>{f.title}</div>
                  </div>
              ...
              {FILMS.map(f=>(
                <div key={f.id} id={`f-${f.id}`} data-film-id={f.id}
                  ...
                  <div style={{width:"100%",aspectRatio:"16/9",background:f.color,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",...(isSnap?{}:{marginBottom:4}),flexShrink:0}}
                    ...
                    onClick={()=>{
                      if(isSnap)setActiveFilm(null);
                      else{setActiveFilm(f);setHf(null);setTimeout(()=>{const el=document.getElementById(`f-${f.id}`);el&&el.scrollIntoView({behavior:"smooth",block:"start"})},50);}
                    }}>
                    <div style={{fontSize:15,fontWeight:700,color:"rgba(255,255,255,.18)",textTransform:"uppercase",letterSpacing:1}}>{f.title}</div>
                  </div>
                  {isSnap&&(
      ...
      {/* ═══ COL 3: YEARS ═══ */}
      <div style={{gridColumn:3,display:"flex",flexDirection:"column",justifyContent:"center",padding:"8px 4px 8px 0"}}>
        {sY.map(y=>{const o=oY(y);return(
          <div key={y} style={{fontSize:FS,lineHeight:LH,textAlign:"right",color:"#fff",fontWeight:o>=1?700:300,opacity:o,transition:"opacity .4s"}}>{y}</div>
        );})}
      </div>
